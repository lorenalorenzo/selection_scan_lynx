...
setwd("/Users/lorenalorenzo/Documents/depth_filter/lp/")
# Create an object with the working directory where there are the depth files.
wd<-"/Users/lorenalorenzo/Documents/depth_filter/lp/"
...
for (i in 1:length(sample_files)){
  ...
  maxDepth= depth_mean + (depth_sd * 2)
  minDepth= depth_mean - (depth_sd * 1.5)
  ...
  depth_per_sample <- rbind(depth_per_sample, data.frame(sample, depth_mean, depth_sd, 
                                 maxDepth, minDepth, PercentLeft))
  write.csv(depth_per_sample, file="lp_depth_table.csv")
